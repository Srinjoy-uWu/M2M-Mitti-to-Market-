        ...
        <div className="max-w-4xl mx-auto">
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="grid md:grid-cols-2 gap-5 mb-5">
                <div className="bg-white rounded-2xl border border-navy-100 shadow-sm p-5 space-y-4">
                  ...
                  <div className="border-t border-navy-50 pt-3">
                    <p className="text-xs text-gray-400 uppercase font-medium tracking-wide mb-2">Confirmations</p>
                    <div className="grid grid-cols-2 gap-2">
                      <div className={`p-2.5 rounded-xl border flex items-center gap-2 text-xs font-semibold ${
                        deal.farmerConfirmed ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-amber-50 border-amber-200 text-amber-800'
                      }`}>
                        {deal.farmerConfirmed ? <CheckCircle2 className="w-4 h-4 text-emerald-600" /> : <Clock className="w-4 h-4 text-amber-600" />}
                        <span>Farmer {deal.farmerConfirmed ? 'Confirmed' : 'Waiting'}</span>
                      </div>
                      <div className={`p-2.5 rounded-xl border flex items-center gap-2 text-xs font-semibold ${
                        deal.buyerConfirmed ? 'bg-emerald-50 border-emerald-200 text-emerald-800' : 'bg-amber-50 border-amber-200 text-amber-800'
                      }`}>
                        {deal.buyerConfirmed ? <CheckCircle2 className="w-4 h-4 text-emerald-600" /> : <Clock className="w-4 h-4 text-amber-600" />}
                        <span>Buyer {deal.buyerConfirmed ? 'Confirmed' : 'Waiting'}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* ═══ FULL-WIDTH LOGISTICS & ROUTE OPTIMIZATION ═══ */}
